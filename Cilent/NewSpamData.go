...
func GetNewSpamData(Deviceid, DeviceName string) []byte {
	...
	wxSaedata03 := DoZlibUnCompress(datas)
	saedatapbdata := &mm.Wcaes{}
	err := proto.Unmarshal(wxSaedata03, saedatapbdata)
	if err == nil {
		...
		var compressdataPtr = (*C.char)(unsafe.Pointer(&compressdata[0]))
		var SaeivPtr = (*C.char)(unsafe.Pointer(&bytesSaeIv[0]))
		var SaetablePtr = (*C.char)(unsafe.Pointer(&SaeTable[0]))
		var SaevaluePtr = (*C.char)(unsafe.Pointer(&SaeValue[0]))
		var SaefinaldataPtr = (*C.char)(unsafe.Pointer(&SaeFinalData[0]))
		var outBuffer = make([]byte, len(compressdata)*2)
		var outbufferPtr = (*C.char)(unsafe.Pointer(&outBuffer[0]))
		var outbufferLen uint32
		var outbufferLenPtr = (*C.uint)(unsafe.Pointer(&outbufferLen))
		cs := C.CString("")
		ret := C.nativewcswbaes(cs, compressdataPtr, C.uint(len(compressdata)), SaeivPtr,
			C.uint(len(bytesSaeIv)), SaetablePtr, C.uint(len(SaeTable)), SaevaluePtr, C.uint(len(SaeValue)),
			SaefinaldataPtr, C.uint(len(SaeFinalData)), 0x3060, outbufferPtr, outbufferLenPtr)
		if ret == 1 {
			return outBuffer[:outbufferLen]
		}
	}
	return nil
}
